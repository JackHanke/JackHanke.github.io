...
About a year after I published the video, I received a comment claiming to have a method for computing $t_{n,m}$ for small fixed $m$. The comment came from a user called Farstar31,

TODO

After some discussions over twitter and email, we worked out a method to compute $t_{n,m}$ for any $n$ and $m$. 
...
where $M_i$ is a sub-matrix (or possible a scalar) of the block matrix $M$. Then $t_{n,m} = ?$
